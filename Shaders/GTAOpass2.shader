Shader "NaiveRP/GTAOpass2"
{
        ...
        SubShader
    {
        ...
        Pass
        {
            ...
            // GTAO
            float _GTAOSpacialFilterSize;
            float _GTAOSpacialFilterVariance;
            ...
            float4 frag(v2f i) : SV_TARGET
            {
                ...
                float weightSum = 0;
                for (int sampleIndex = -12; sampleIndex <= 12; sampleIndex++) {
                    float2 pixelDisplacement = float2(0, sampleIndex);
                    float2 sampleUV = i.uv + (pixelDisplacement.x * sampleBasis1 + pixelDisplacement.y * sampleBasis2) * _GTAOSpacialFilterSize;
                    float4 sampleValue = tex2D(_GTAOResolved, sampleUV);
                    float3 sampleNormal = normalize(tex2D(_GT1, sampleUV).rgb * 2 - 1);
                    float weight = exp(-dot(pixelDisplacement, pixelDisplacement) / _GTAOSpacialFilterVariance) * saturate(dot(normal, sampleNormal));
                    filteredValue += weight * sampleValue.r;
                    weightSum += weight;
                }
                filteredValue /= weightSum;

                return float4(filteredValue, 0, 0, 1);
            }
            ENDCG
        }
    }
