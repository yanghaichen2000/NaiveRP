Shader "NaiveRP/postprocesspass"
{
    Properties
    {
        ...
        SubShader
    {
        ...
        Pass
        {
            ...
            {
                float3 sceneColor = tex2D(_sceneColor, i.uv).rgb;
                ...
                // SSR
                float4 SSRRadiance_AvailableRatio = tex2D(_SSRFilteredRadiance, i.uv);
                float3 SSRColor = SSRRadiance_AvailableRatio.rgb;
                float SSRAvailableRatio = SSRRadiance_AvailableRatio.a;
                //SSRAvailableRatio = SSRAvailableRatio * 0.6; // adjust
                if (metallic > 0.01) {
                    col = col * pow(1 - SSRAvailableRatio, 0.4) + SSRColor * SSRAvailableRatio * 0.7;
                }

                // GTAO
                float GTAOVisibility = tex2D(_GTAOResolved, i.uv).x;
                GTAOVisibility = lerp(1, GTAOVisibility, _GTAOStrength);
                col *= GTAOVisibility;
