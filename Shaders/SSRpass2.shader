Shader "NaiveRP/SSRpass2"
{
        ...
        SubShader
    {
        ...
        Pass
        {
            ...
            Texture2D _SSRPDF;
            SamplerState _sampler_point_clamp_SSRPDF;
            Texture2D _SSRH;
            SamplerState _sampler_point_clamp_SSRH;
            sampler2D _SSRResolvedRadiance;


            float3 SpecularPBRShading(float3 normal, float3 view_dir, float3 light_dir, float3 light_color, float roughness, float3 albedo, float metallic) {
                ...
                return (specular_color) * UNITY_PI;
            }
            ...
            float4 frag(v2f i) : SV_TARGET
            {
                ...
                float dU = _screenPixelWidthInv;
                float dV = _screenPixelHeightInv;
                float2 dUV[9] = {
                    float2(0, 0),
                    float2(dU, 0),
                    float2(0, dV),
                ...
                int sampleNum = 1;
                float3 SSRColorSum = float3(0, 0, 0);
                float hitSum = 0;
                for (int sampleIndex = 0; sampleIndex < sampleNum; sampleIndex++) {
                    float2 sampleUV = i.uv + dUV[sampleIndex];
                    float4 sampleHitData = _SSRData.Sample(_sampler_point_clamp_SSRData, sampleUV);
                    float2 sampleHitUV = sampleHitData.xy;
                    hitSum += sampleHitData.a;
                    float3 H = _SSRH.Sample(_sampler_point_clamp_SSRH, sampleUV).xyz * 2 - 1;
                    float3 L = reflect(-V, H);
                    float PDF = _SSRPDF.Sample(_sampler_point_clamp_SSRPDF, sampleUV);
                    float coneIntersectionCircleRadius = coneTangent * length(sampleHitUV - sampleUV);
                    liRadianceMip = clamp(log2(coneIntersectionCircleRadius * 1000), 0, 4);
                    float3 liRadiance = tex2Dlod(_sceneColor, float4(sampleHitUV, 0, liRadianceMip));
                    SSRColorSum += SpecularPBRShading(normal, V, L, liRadiance, roughness, albedo, metallic) / max(0.00001, PDF);
                }

                float3 SSRColor = SSRColorSum / sampleNum;

                return float4(SSRColor, hitSum / sampleNum);
            }
            ENDCG
        }
    }
